...
void Cheats::Menu()
{
	...
	ImGui::Begin("VSS PROJECT CS2", nullptr, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize);
	{
		...
		if (ImGui::BeginChild(2, ImVec2(485, 600), true)) {
			...
			if (tabb == 0) {
				...
				if (MenuConfig::ESPPreview) {
					...
					const auto& window = ImGui::FindWindowByName("VSS PROJECT CS2");
				...
				ImGui::Combo("Select Style", &MenuConfig::selectedStyleIndex, "Purple\0Future Dark\0Green\0");
				...
				ImGui::Checkbox("VAC Secure", &MenuConfig::OBSBypass);
				ImGui::Checkbox("Teleport ON/OFF", &MenuConfig::OBSBypass);
				ImGui::SameLine();
				ImGui::Checkbox("Spectate Esp", &MenuConfig::SpectateEsp);
				if (ImGui::Button("Discord")) {
					std::system("cmd.exe /c start https://discord.gg/zT9yxkHDg5");
				}
				ImGui::Text("[INSERT] HideMenu");
			}
		}
